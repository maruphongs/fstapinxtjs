...
export default function ProductPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await Promise.all([
        ...form.categoryIds.filter((id) => !previousCategoryIds.includes(id)).map((categoryId) => fetch(`${API_URL}/products/${saved.id}/categories/${categoryId}`, { method: "POST" })),
        ...previousCategoryIds.filter((id) => !form.categoryIds.includes(id)).map((categoryId) => fetch(`${API_URL}/products/${saved.id}/categories/${categoryId}`, { method: "DELETE" })),
      ]);
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-10 font-sans text-slate-900 sm:px-6 sm:py-16">
      <div className="mx-auto max-w-6xl">
        <header className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button type="button" className="w-full bg-black px-5 py-3 font-bold text-white transition hover:bg-[#26a699] sm:w-auto" onClick={openAddProduct}>+ Add product</button>
        </header>
